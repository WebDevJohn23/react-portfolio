...
function getDescription(item) {
  const raw = item?.excerpt?.rendered ?? item?.acf?.description ?? item?.acf?.summary ?? '';
  return typeof raw === 'string' ? raw : '';
}
        ...
        {projects.map((p) => {
          ...
          return (
            <article key={p.id ?? p.slug} style={{ border: '1px solid #eee', padding: 12 }}>
              <h3 dangerouslySetInnerHTML={{ __html: title }} style={{ marginTop: 0 }} />
              {img && (
                <img
                  src={img}
              ...
              {desc && <p dangerouslySetInnerHTML={{ __html: desc }} style={{ marginBottom: 0 }} />}
            </article>
          );
        })}
      </div>
